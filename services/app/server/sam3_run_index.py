...
def _effective_prompt(segment_dir: Path) -> tuple[dict, str]:
    ...
    override_path = segment_dir / "_sam3" / "prompt_override.json"
...
def index_completed_runs(connection, *, object_id: str, relpath: str, result: dict) -> int:
    ...
    with connection.cursor() as cursor:
        ...
        for summary in segment_runs:
            ...
            parameters = summary.get("params") or {}
            cursor.execute(
                """
            ...
            if row:
                ...
                cursor.execute("SELECT count(*) FROM frame_instances WHERE run_id=%s", (run_id,))
                if int(cursor.fetchone()[0]) == frame_count * len(prompt.get("objects") or []):
                    continue
            ...
            for obj in prompt.get("objects") or []:
                ...
                for frame_idx in range(frame_count):
                    mask = segment_dir / "_sam3" / "masks" / str(obj_id) / f"{frame_idx:06d}.png"
                    info = inspect_binary_png(mask.read_bytes(), expected_size=(width, height))
    ...
    *,
    ...
    if entry["status"] == "edited":
        for item in entry.get("instances") or []:
            ...
            mask = segment_dir / "_sam3" / item["path"]
    ...
    *,
    ...
    with psycopg.connect(database_url) as connection, connection.cursor() as cursor:
        cursor.execute(
            ...
            """,
            (object_id, relpath, start, end, digest),
        )
        row = cursor.fetchone()
        if row is None:
